...
function releasesFor(sel){ if(sel==='all'){ const arr=[]; Object.keys(LIBRARY).forEach(c=>LIBRARY[c].forEach((rel,ri)=>arr.push({cat:c,ri,rel}))); arr.sort((a,b)=>a.rel.date.localeCompare(b.rel.date)); return arr; } return (LIBRARY[sel]||[]).map((rel,ri)=>({cat:sel,ri,rel})); }
